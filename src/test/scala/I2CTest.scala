...
class I2CTest
  ...
  def runTest(name: String): Unit = {
    ...
    val myParams = BaseParams(dataWidth=8, addrWidth = 16, regWidth = 8, clkFreq = 50, coverage = true)
    ...
    name match {
      ...
      case "formal_mm" =>
        "MultiMasterI2C" should "Formally Verify" in
          verify(
            new MultiMasterI2C(BaseParams(8, 16), true), 
            Seq(BoundedCheck(20)))

      case "formal_fullduplex" =>
        "FullDuplexI2C" should "Formally Verify" in
          verify(
            new FullDuplexI2C(BaseParams(8, 16), true), 
            Seq(BoundedCheck(20)))
